...
def create_dataset(X,Y):
    ...
    y_oblik=(broj_primera-vremenska_duzina,1)
...
plt.plot(range(X_test.shape[0]),X_test[::,50,0],'b')
plt.show()
plt.plot(range(X_test.shape[0]),X_test[::,50,1],'r')
plt.show()
model=Sequential()
...
model.add(Dropout(0.2))
model.add(LSTM(units=20,activation='tanh',recurrent_activation='hard_sigmoid',recurrent_initializer='glorot_uniform'))
model.add(Dropout(0.2))
model.add(Dense(units=1,activation='sigmoid'))
model.compile(optimizer='adam',loss='binary_crossentropy',metrics=['acc',f1])
path_checkpoint='najbolje_tezine_za_EKG.keras'
